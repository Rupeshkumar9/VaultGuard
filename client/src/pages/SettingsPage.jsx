...
export default function SettingsPage() {
  ...
  const handleOpenWebDashboard = async () => {
    setSyncError('');
    try {
      const response = await chrome.runtime.sendMessage({ action: 'OPEN_FRONTEND' });
      if (!response?.success) {
        throw new Error(response?.error || 'The web dashboard URL is not configured.');
      }
    } catch (err) {
      console.error('Failed to open web dashboard:', err);
      setSyncError(err.message || 'Failed to open web dashboard.');
    }
  };
